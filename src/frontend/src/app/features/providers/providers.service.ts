...
export class ProvidersService {
  ...
  public fetchProviders() {
    ...
    return this.providersApiClient.getProviders()
      .pipe(
        tap(providers => {
    ...
    return this.providersApiClient
      ...
      .pipe(
        tap(provider => {
          ...
          if (index < 0) {
            // refresh?
            console.log('Something went wrong: No data provider in providers list after update')
            return
          }
  ...
  removeProvider(providerId: string) {
    this._isLoading.set(true);
    return this.providersApiClient.removeProvider(providerId)
      .pipe(
        tap(_ => {
          const providers = this._providers.value;
          ...
          if (index < 0) {
            // refresh?
            console.log('Something went wrong: No data provider in providers list after remove')
            return
          }
