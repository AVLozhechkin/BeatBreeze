...
export class PlayerService {
  ...
  public async playSongs(source: Song[]) {
    ...
    const song = this.queueService.getSongByIndex(this.queueService.currentIndex())!
  ...
  private async startPlaying(song: Song)
  {
    ...
    howl.once('end', () => this.checkNext())
  }

  public play() {
    if (this._playState() === 'playing' || this._playState() === 'unloaded' || this._currentHowl()?.playing())
      return
  ...
  public pause() {
    if (this._playState() === 'paused' || this._playState() === 'unloaded' || this._playState() === 'idle' || !this._currentHowl()!.playing() || this._locked()) {
      return;
    }
  ...
  private updateCurrentProgress() {
    ...
    this._songDuration.set(PlayerService.convertSeekToTime(this._currentHowl()!.duration()))
    this._seek.set(PlayerService.convertSeekToTime(this._currentHowl()!.seek()));
    this._songProgress.set(`${(this._currentHowl()!.seek() / this._currentHowl()!.duration()) * 100}%`)
  ...
  private async checkNext() {
    this._playState.set('idle')
    console.log(this.repeatType() + ' left: ' + this.queueService.songsLeft() + ' index: ' + this.queueService.currentIndex())
    if (this.repeatType() === 'repeat-1') {
      this.play()
      return;
    }

    if (this.repeatType() === 'repeat-none' && this.queueService.songsLeft() === 1)
    {
      return;
    }
