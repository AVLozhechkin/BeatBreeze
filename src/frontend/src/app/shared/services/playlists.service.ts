...
export class PlaylistsService {
  ...
  public fetchPlaylists() {
    ...
    this.playlistsApiClient.getUserPlaylists().subscribe({
      next: (playlists) => {
  ...
  addSongToPlaylist(playlistId: string, songFileId: string) {
    this.playlistsApiClient.addSong(playlistId, songFileId).subscribe();
  }
}
