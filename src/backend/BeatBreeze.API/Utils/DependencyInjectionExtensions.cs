...
using BeatBreeze.API.Exceptions.Handlers;
...
public static class DependencyInjectionExtensions
{
    ...
    {
        var isProxiedVariable = Environment.GetEnvironmentVariable("IsProxied");

        if (bool.TryParse(isProxiedVariable, out bool isProxied) && isProxied)
        {
    ...
    private static void SetupAuthentication(this IServiceCollection services, ConfigurationManager configuration)
    {
        services.AddAuthentication(CookieAuthenticationDefaults.AuthenticationScheme)
            ...
            {
                options.Cookie.Name = "BeatBreeze";
                options.Cookie.MaxAge = TimeSpan.FromDays(7);
                options.Cookie.HttpOnly = false;
            })
            .AddCookie("External", options =>
            {
                options.Cookie.Name = "BeatBreeze_External";
            })
            .AddYandex(options =>
            {
